<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        if ($_POST['type'] == "create") {
            ...
            $time = time();
            $insert = $con->prepare("INSERT INTO vehicles (citizenid,fullname,avatar,plate,typevehicle,note,lastsearch) 
                                                VALUES(:citizenId, :fullname, :avatar, :plate, :typevehicle, :note, :lastsearch);");
            $insert->bindParam(":citizenId", $_POST['citizenid']);
            $insert->bindParam(":fullname", $_POST['fullname']);
            $insert->bindParam(":avatar", $_POST['avatar']);
            $insert->bindParam(":plate", $_POST['plate']);
            $insert->bindParam(":typevehicle", $_POST['typevehicle']);
            $insert->bindParam(":note", $note);
            $insert->bindParam(":lastsearch", $time);
            if ($insert->execute()) {
                $_SESSION["vehicleid"] = $con->lastInsertId();
                $respone = true;
            ...
            $note = nl2br($_POST["note"]);
            $update = $con->prepare("UPDATE vehicles SET citizenid = :citizenId, fullname = :fullname, avatar = :avatar, plate = :plate, typevehicle = :typevehicle, note = :note WHERE id = :vehicleId");
            $update->bindParam(":citizenId", $_POST['citizenid']);
            $update->bindParam(":fullname", $_POST['fullname']);
            $update->bindParam(":avatar", $_POST['avatar']);
            $update->bindParam(":plate", $_POST['plate']);
            $update->bindParam(":typevehicle", $_POST['typevehicle']);
            $update->bindParam(":note", $note);
            $update->bindParam(":vehicleId", $_POST['vehicleid']);
            if ($update->execute()) {
    ...
    <body>
        ...
        <main role="main" class="container">
            ...
            <?php if (isset($_GET['delete']) && $_GET['delete'] != "") { 
                $deleteVehicle = $con->prepare("DELETE FROM vehicles WHERE id = :vehicleId");
                $deleteVehicle->bindParam(":vehicleId", $_GET["delete"]);
                $deleteVehicle->execute();
            ?>
            <script>window.location = '/databank/dashboard';</script>
            <?php
            } else if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['type'] == "edit" && !empty($selectedvehicle)) { ?>
                <form method="post">
                    <input type="hidden" name="type" value="realedit">
                    <input type="hidden" name="vehicleid" value="<?php echo $selectedvehicle["id"]; ?>">
                    <div class="input-group mb-3">
                        <input type="text" name="citizenid" class="form-control login-user" value="<?php echo $selectedvehicle["citizenid"]; ?>" placeholder="bsn" required>
                    </div>
                    <div class="input-group mb-2">
                        <input type="text" name="fullname" class="form-control login-pass" value="<?php echo $selectedvehicle["fullname"]; ?>" placeholder="volledige naam" required>
                    </div>
                    <div class="input-group mb-3">
                        <input type="text" name="avatar" class="form-control login-user" value="<?php echo $selectedvehicle["avatar"]; ?>" placeholder="profiel foto (imgur URL vb. https://i.imgur.com/zKDjdhe.png)" required>
                    </div>
                    <div class="input-group mb-3">
                        <input type="text" name="plate" class="form-control login-user" value="<?php echo $selectedvehicle["plate"]; ?>" placeholder="kenteken">
                    </div>
                    <div class="input-group mb-3">
                        <input type="text" name="typevehicle" class="form-control login-user" value="<?php echo $selectedvehicle["typevehicle"]; ?>" placeholder="voertuig type">
                    </div>
                    <?php $notes = str_replace( "<br />", '', $selectedvehicle["note"]); ?>
                    <div class="input-group mb-2">
            ...
            <?php } else { ?>
                <form method="post">
                    ...
                    <div class="input-group mb-3">
                        <input type="text" name="plate" class="form-control login-user" value="" placeholder="kenteken">
                    </div>
                    <div class="input-group mb-3">
                        <input type="text" name="typevehicle" class="form-control login-user" value="" placeholder="voertuig type">
                    </div>
                    <div class="input-group mb-2">
